...
public class WeixinServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			System.out.println("发送的类型：" + msgType);
			String message = null;
			if (MessageUtil.MESSAGE_TEXT.equals(msgType)) {//自定义关键词回复
				if ("1".equals(content)) {
					...
					message = MessageUtil.initText(toUserName, fromUserName, MessageUtil.repeat(content));
				}
			} else if (MessageUtil.MESSAGE_EVNET.equals(msgType)) {//关注后自动回复
